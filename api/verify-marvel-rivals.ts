...
function mapMarvelRankToDiscord(rank: string, tier: number): string | null {
  ...
  if (normalized.includes('one above all')) return 'X';
  if (normalized.includes('eternity')) return 'CHALLENGER V';

  if (normalized.includes('bronze')) return mapTierToRank(['GRNDS I', 'GRNDS II', 'GRNDS III'], tier);
  if (normalized.includes('silver')) return mapTierToRank(['GRNDS IV', 'GRNDS V', 'GRNDS V'], tier);
  if (normalized.includes('gold')) return mapTierToRank(['BREAKPOINT I', 'BREAKPOINT II', 'BREAKPOINT II'], tier);
  if (normalized.includes('platinum')) return mapTierToRank(['BREAKPOINT III', 'BREAKPOINT IV', 'BREAKPOINT IV'], tier);
  if (normalized.includes('diamond')) return mapTierToRank(['BREAKPOINT V', 'CHALLENGER I', 'CHALLENGER I'], tier);
  if (normalized.includes('grandmaster')) return mapTierToRank(['CHALLENGER II', 'CHALLENGER III', 'CHALLENGER III'], tier);
  if (normalized.includes('celestial')) return mapTierToRank(['CHALLENGER IV', 'CHALLENGER V', 'CHALLENGER V'], tier);
